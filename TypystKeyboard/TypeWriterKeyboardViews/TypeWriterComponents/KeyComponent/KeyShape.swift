...
struct KeyShape {
    ...
    //    private static func modifyPoint(_ point: CGPoint, keySegment: KeySegment, inset: CGFloat = 0.0) -> CGPoint {
    //        switch(keySegment) {
    //            case .topLine: return CGPoint(x: point.x - inset, y: point.y + inset)
    //            case .topLine: return CGPoint(x: point.x + inset, y: point.y + inset)
    //            case .topRightCorner: return CGPoint(x: point.x + inset, y: point.y + inset)
    //            case .topLine: return CGPoint(x: point.x + inset, y: point.y + inset)
    //            case .topLine: return CGPoint(x: point.x + inset, y: point.y + inset)
    //            case .topLine: return CGPoint(x: point.x + inset, y: point.y + inset)
    //            case .topLine: return CGPoint(x: point.x + inset, y: point.y + inset)
